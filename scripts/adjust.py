...
energy_gev = data.T[0] * (10**(-3)) # get to units of GeV

xs_uneven = (data.T[1] * 10**(-4)) # match units to get to 10^-38 cm^2

#new_x = np.linspace(min(col_1), max(col_1), 1001)

# make evenly spaced bins from 0 -> 0.1 GeV
new_x = np.linspace(0, 0.1, 1001) # 0 -> 0.1 GeV

# overwrite first entry so as not to div by zero
# need to update later to start at 5 MeV
new_x[0] = 10**(-10)
...
interp_xs = np.interp(new_x, energy_gev, xs_uneven)
xs = [i/j for i,j in zip(interp_xs, new_x)]
